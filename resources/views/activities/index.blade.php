<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center px-4 py-2 font-medium text-white rounded-lg shadow transition bg-gradient-to-r from-emerald-500 to-emerald-700">
    ...
    @php
        ...
        $filterOptions = [
            ...
            'add' => ['label' => 'Penambahan', 'icon' => 'fa-circle-plus'],
            'edit' => ['label' => 'Pembaruan', 'icon' => 'fa-pen-to-square'],
    ...
    <div class="space-y-6 pb-12" x-data="activityPage()">
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <span class="inline-flex items-center justify-center w-8 h-8 bg-emerald-100 text-emerald-600 rounded-full flex-shrink-0">
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <span class="inline-flex items-center justify-center w-8 h-8 bg-red-100 text-red-600 rounded-full flex-shrink-0">
        ...
        <div class="grid gap-4 grid-cols-2 lg:grid-cols-4">
            <div class="p-4 sm:p-5 bg-white rounded-2xl shadow-md ring-1 ring-emerald-100">
                <div class="flex items-center gap-3 sm:gap-4">
                    <span class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-emerald-100 text-emerald-600 rounded-xl flex-shrink-0">
            ...
            <div class="p-4 sm:p-5 bg-white rounded-2xl shadow-md ring-1 ring-emerald-100">
                <div class="flex items-center gap-3 sm:gap-4">
                    <span class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-sky-100 text-sky-600 rounded-xl flex-shrink-0">
            ...
            <div class="p-4 sm:p-5 bg-white rounded-2xl shadow-md ring-1 ring-emerald-100">
                <div class="flex items-center gap-3 sm:gap-4">
                    <span class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-amber-100 text-amber-600 rounded-xl flex-shrink-0">
            ...
            <div class="p-4 sm:p-5 bg-white rounded-2xl shadow-md ring-1 ring-emerald-100">
                <div class="flex items-center gap-3 sm:gap-4">
                    <span class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-teal-100 text-teal-600 rounded-xl flex-shrink-0">
                        <i class="text-base sm:text-lg fas fa-user-check"></i>
                    </span>
                    <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <form action="{{ route('activities.clear') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus seluruh riwayat aktivitas? Tindakan ini tidak dapat dibatalkan.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-red-500 to-red-600 rounded-xl hover:scale-[1.02] shadow-lg shadow-red-400/30 transition">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div x-data="datePicker('date_from', '{{ $dateFrom }}')" class="relative" @click.away="closeAll()">
                            ...
                            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
                                ...
                                <div class="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-t-2xl">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div x-data="{ open: false, selected: {{ (int)$perPage }} }" class="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-200">
                        <label class="text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">
                            <i class="fas fa-list-ol mr-1 text-emerald-500"></i>Tampilkan:
                        </label>
                        <div class="relative">
                            <button type="button" @click="open = !open" @click.away="open = false"
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0 space-y-2.5">
                                    ...
                                    <div class="flex items-start justify-between gap-2">
                                        <div class="flex items-center gap-2">
                                            <span class="inline-flex items-center justify-center w-8 h-8 text-emerald-600 bg-emerald-50 rounded-lg flex-shrink-0">
                                        ...
                                        <span class="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-semibold rounded-md {{ $palette['classes'] }} flex-shrink-0">
                                ...
                                <th class="px-5 py-4 border border-emerald-500">
                                    <a href="{{ route('activities.index', array_merge(request()->query(), ['sort' => 'user', 'dir' => ($sortBy === 'user' && $sortDir === 'asc') ? 'desc' : 'asc'])) }}" 
                                       class="flex items-center gap-2 hover:text-emerald-100 transition">
                                        Pengguna
                                        @if($sortBy === 'user')
                                            <i class="fas fa-sort-{{ $sortDir === 'asc' ? 'up' : 'down' }} text-emerald-200"></i>
                                        @else
                                            <i class="fas fa-sort text-emerald-300"></i>
                                        @endif
                                    </a>
                                </th>
                                <th class="px-5 py-4 border border-emerald-500">
                            ...
                            @forelse ($activities as $index => $activity)
                                ...
                                <tr class="transition hover:bg-emerald-50/60 group {{ $index % 2 === 0 ? 'bg-white' : 'bg-slate-50' }}">
                                    ...
                                    <td class="px-5 py-4 align-middle border border-slate-200">
                                        <div class="flex items-center gap-3">
                                            <span class="inline-flex items-center justify-center w-9 h-9 text-emerald-600 bg-emerald-50 rounded-lg group-hover:scale-105 transition flex-shrink-0">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-emerald-100 bg-white">
                    ...
                    <nav class="flex items-center gap-1">
                        ...
                        @for ($page = $start; $page <= $end; $page++)
                            @if ($page == $currentPage)
                                <span class="inline-flex items-center justify-center w-9 h-9 text-sm font-bold text-white bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-lg shadow-md shadow-emerald-400/30">{{ $page }}</span>
                            @else
